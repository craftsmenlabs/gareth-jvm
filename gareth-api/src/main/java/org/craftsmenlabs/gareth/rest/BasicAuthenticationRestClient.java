...
public class BasicAuthenticationRestClient {
    ...
    BasicAuthenticationRestClient(RestTemplate template,
                                  String userName,
                                  String password,
    ...
    public <T extends Serializable> T get(Class<T> cls, String url) {
        return getAsEntity(cls, url).getBody();
    }

    public <T extends Serializable> ResponseEntity<T> getAsEntity(Class<T> cls, String url) {
        return exchange(HttpMethod.GET, url, Optional.empty(), cls);
    }

    public <T extends Serializable> ResponseEntity<T> putAsEntity(Serializable body, Class<T> cls, String url) {
        return exchange(HttpMethod.PUT, url, Optional.of(body), cls);
    }

    public <T extends Serializable> T put(Serializable body, Class<T> cls, String url) {
        return putAsEntity(body, cls, url).getBody();
    }

    public <T extends Serializable> ResponseEntity<T> post(String url, Class<T> cls) {
        return exchange(HttpMethod.POST, url, Optional.empty(), cls);
    }

    public <T extends Serializable> T post(Serializable body, Class<T> cls, String url) {
        return postAsEntity(body, cls, url).getBody();
    }

    public <T extends Serializable> ResponseEntity<T> postAsEntity(Serializable body, Class<T> cls, String url) {
    ...
    private <T> ResponseEntity<T> exchange(HttpMethod method, String url, Optional<Serializable> body, Class<T> cls) {
        ...
        try {
            ...
            if (getErrorHandler().hasError()) {
                LOGGER.error("REST call to {} returned {} error. Message: {} ",
                        methodPlusUrl,
    ...
    private <T> HttpEntity<T> createEntity(HttpHeaders requestHeaders, Optional<T> body) {
        if (basicAuthenticationHeaders == null) {
            throw new IllegalStateException("No authentication information present: " +
                    "When using the get/put/post/delete methods without username and password arguments, you must provide them during construction");
        }
