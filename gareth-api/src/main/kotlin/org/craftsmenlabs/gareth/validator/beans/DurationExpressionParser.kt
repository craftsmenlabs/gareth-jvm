...
import org.craftsmenlabs.gareth.validator.time.TimeUnit
 ...
 */
@Service
open class DurationExpressionParser @Autowired constructor(private val dateTimeService: TimeService) {
    ...
    fun calculateTimeDifferenceFromNow(duration: Duration): LocalDateTime {
        return dateTimeService.now().plus(duration)
    }
    ...
    private fun addDurationToNow(duration: Duration) = dateTimeService.now().plus(duration)
